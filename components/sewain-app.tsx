    ...
    <section className="panel property-overview">
      <div className="property-overview-main">
        ...
        <div className="property-overview-copy">
          <div className="property-header-labels">{propLabels.map(label => <span key={label}>{v(label)}</span>)}{property.genderRestriction ? <span className="gender-badge">{t(String(property.genderRestriction))}</span> : null}</div>
...
function PropertyDialog({ state, onClose, onSave }: { state: Exclude<DialogState, null>; onClose: () => void; onSave: (page: PageId, row: Row) => void }) {
  ...
  const [genderRestriction, setGenderRestriction] = useState(String(row?.genderRestriction || ""));
  ...
  return <div className="backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && onClose()}><form className="dialog property-dialog" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="property-form-title">
    ...
    <div className="dialog-body property-form">
      ...
      <section className="form-section"><div className="form-section-head"><strong>{t("Ketentuan penghuni")}</strong><span>2b</span></div><div className="form-field full"><label htmlFor="gender-restriction">{t("Pembatasan gender")}</label><select id="gender-restriction" value={genderRestriction} onChange={event => setGenderRestriction(event.target.value)}><option value="">{t("Campur (putra & putri)")}</option><option value="Putra">{t("Putra saja")}</option><option value="Putri">{t("Putri saja")}</option></select><small className="field-hint">{t("Membatasi penyewa yang dapat booking ke properti ini.")}</small></div></section>
...
function BookingDialog({ ctx, properties, units, setUnits, tenants, setTenants, reservations, setReservations, onClose, onCreated, notify }: BookingDialogProps) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    const restriction = String(selectedProperty.genderRestriction || "");
