...
function TicketDragPreview({ preview }: { preview: Exclude<TicketDragPreviewState, null> }) {
  ...
  return <div className="ticket-drag-preview" aria-hidden="true" style={{ width: preview.width, minHeight: preview.height, transform: `translate3d(${preview.x}px, ${preview.y}px, 0) rotate(.6deg) scale(1.02)` }}>
    ...
    <div className="ticket-footer"><span className={`vendor-chip ${row.vendor === "Belum ditugaskan" ? "unassigned" : ""}`}><Wrench />{v(row.vendor)}</span>{row.dueDate && <span className="ticket-due"><CalendarClock />{v(row.dueDate)}</span>}{row.laporanVendor ? <span className="ticket-report-flag"><CheckCircle2 />{locale === "en" ? "Report submitted" : "Laporan terkirim"}{row.tanggalSelesai && <small> · {v(row.tanggalSelesai)}</small>}</span> : null}</div>
  </div>;
}
